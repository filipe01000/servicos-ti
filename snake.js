(() => {
 ...
 try{best=Math.max(0,Number(localStorage.getItem('fa-snake-best'))||0);}catch(_){}bestEl.textContent=best;
 ...
 function tick(){dir=next;queued=false;const head={x:snake[0].x+dir.x,y:snake[0].y+dir.y};const eating=head.x===food.x&&head.y===food.y;const body=eating?snake:snake.slice(0,-1);if(head.x<0||head.x>=size||head.y<0||head.y>=size||body.some(p=>p.x===head.x&&p.y===head.y)){finish();return;}snake.unshift(head);if(eating){score+=10;scoreEl.textContent=score;if(score>best){best=score;bestEl.textContent=best;try{localStorage.setItem('fa-snake-best',String(best));}catch(_){}}food=spawn();if(!food){draw();finish(true);return;}}else snake.pop();draw();}
